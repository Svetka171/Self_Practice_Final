...
public class CombinationOfUSCoins_HELP {
     ...
     */
        System.out.println("combo(11) = " + countWaysToProduceGivenAmountOfMoney(11));
    }
    ...
    public static int countWaysToProduceGivenAmountOfMoney(int cents) {
        ...
        for (int i = 1; i <= coins.length; i++) {    //i<5      i=1
            for (int j = 1; j <= cents; j++) {      //j<11      j=1
                // check if the coin value is less than the amount needed
                if (coins[i - 1] <= j) {  //  coins[0] (1)<=j
                    // reduce the amount by coin value and
                    // use the subproblem solution (amount-v[i]) and
                    // add the solution from the top to it
                    s[i][j] = s[i - 1][j] + s[i][j - coins[i - 1]];   // s[0][1] + s[1][0];
                } else {
